...
class HexEngine:
    ...
    def get_hex_dump(file_path, offset=0, page_size=4096, physical_mode=False):
        ...
        try:
            ...
            return "".join(output)
        except: return "[-] Read Error: Access Denied (Locked Artifact)."

# --- 4. ANALYSIS POPUP WINDOW ---
class AnalysisWindow(ctk.CTkToplevel):
    def __init__(self, title, columns, data):
        super().__init__()
        self.title(title); self.geometry("1200x700"); self.attributes('-topmost', True)
...
class BrowsAR_App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Brows-AR | Ultimate Forensic Suite"); self.geometry("1450x900")
        ctk.set_appearance_mode("dark")
        
        self.current_browser_path = ""
        self.current_browser_name = ""
        self.physical_mode = False 
        self.tab_offsets = {}; self.page_size = 4096; self.displays = {}
        self.last_results = []; self.last_headers = []

        self.sidebar = ctk.CTkFrame(self, width=240, corner_radius=0); self.sidebar.pack(side="left", fill="y")
        ...
        self.main_view = ctk.CTkFrame(self, corner_radius=15, fg_color="#121212"); self.main_view.pack(side="right", fill="both", expand=True, padx=20, pady=20)
    ...
    def load_browser_cockpit(self, name, path):
        ...
        control_frame = ctk.CTkFrame(self.main_view, fg_color="transparent"); control_frame.pack(fill="x", padx=20, pady=15)
        ...
        for t in tabs: 
            self.tabview.add(t); self.tab_offsets[t] = 0
    ...
    def setup_hex_tab(self, t):
        container = ctk.CTkFrame(self.tabview.tab(t), fg_color="transparent"); container.pack(fill="both", expand=True)
        txt = ctk.CTkTextbox(container, font=("Consolas", 14), wrap="none", fg_color="#000000", text_color="#ffffff")
        txt.pack(fill="both", expand=True, padx=10, pady=(10, 5)); self.displays[t] = txt
        nav = ctk.CTkFrame(container, fg_color="transparent", height=40); nav.pack(fill="x", side="bottom", padx=10, pady=5)
    ...
    def setup_registry_explorer(self, t):
        container = ctk.CTkFrame(self.tabview.tab(t), fg_color="#1a1a1a"); container.pack(fill="both", expand=True, padx=10, pady=10)
        self.reg_tree = ttk.Treeview(container, show="tree"); self.reg_tree.pack(side="left", fill="both", expand=True)
        ...
        self.reg_values.pack(side="right", fill="both", expand=True); self.refresh_registry_tree()

    def get_reg_targets(self):
        if self.current_browser_name == "Brave":
            return [(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\BraveSoftware"), (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\WOW6432Node\BraveSoftware"), (winreg.HKEY_CURRENT_USER, r"Software\BraveSoftware"), (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Brave-Browser"), (winreg.HKEY_CLASSES_ROOT, r"BraveHTML")]
        elif self.current_browser_name == "Chrome":
            return [(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Google\Chrome"), (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\WOW6432Node\Google\Update\ClientState\{8A69D345-D564-463c-AFF1-A69D9E530F96}"), (winreg.HKEY_CURRENT_USER, r"Software\Google\Chrome")]
        else:
            return [(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Edge"), (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\WOW6432Node\Microsoft\EdgeUpdate\ClientState\{56EB18F8-8008-4CBD-B6D2-8C97FE7E9062}"), (winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Edge")]
    ...
    def refresh_registry_tree(self):
        ...
        for root, path in self.get_reg_targets():
            ...
            node = self.reg_tree.insert("", "end", text=f"{r_name}: {path}", values=(root, path)); self.add_reg_subkeys(node, root, path)
    ...
    def on_registry_key_selected(self, event):
        selected = self.reg_tree.selection()[0]; root, path = self.reg_tree.item(selected, "values")
        ...
        try:
            with winreg.OpenKey(int(root), path, 0, winreg.KEY_READ | winreg.KEY_WOW64_64KEY) as key:
                for i in range(winreg.QueryInfoKey(key)[1]):
                    n, d, _ = winreg.EnumValue(key, i); self.reg_values.insert("", "end", values=(n, str(d)))
        except: pass
    ...
    def on_tab_changed(self):
        ...
        if path: txt.insert("end", HexEngine.get_hex_dump(path, offset=self.tab_offsets.get(active, 0), physical_mode=self.physical_mode))
        ...
        def crawl(root, path):
            try:
                with winreg.OpenKey(root, path, 0, winreg.KEY_READ | winreg.KEY_WOW64_64KEY) as key:
                    ...
                    for i in range(n_v): n, v, _ = winreg.EnumValue(key, i); res.append((f"  -> {n}", str(v)))
                    for i in range(n_s): crawl(root, rf"{path}\{winreg.EnumKey(key, i)}")
            except: pass
        for r, p in targets: crawl(r, p)
        self.last_results, self.last_headers = res, ["Artifact Path", "Value / Metadata"]
        AnalysisWindow(f"{self.current_browser_name} Comprehensive Registry Scan", self.last_headers, res)
    ...
    def analyze_passwords(self):
        ...
        if "app_bound_encrypted_key" in ls["os_crypt"]:
            ...
            if impersonate_system():
                try: s1 = win32crypt.CryptUnprotectData(raw_key, None, None, None, 0)[1]
        ...
        for url, u_v, enc_p in cur.fetchall():
            ...
            except: res.append((url, u_v, "[Encrypted]"))
        conn.close(); os.remove(temp_db); self.last_results, self.last_headers = res, ["URL", "User", "Password"]
        AnalysisWindow(f"{self.current_browser_name} Credentials", self.last_headers, res)

    def analyze_history(self):
        path = self.get_artifact_path("History"); temp = "temp_hist.db"; shutil.copyfile(path, temp)
        conn = sqlite3.connect(temp); cur = conn.cursor(); cur.execute("SELECT url, title, visit_count FROM urls")
        data = cur.fetchall(); conn.close(); os.remove(temp); self.last_results, self.last_headers = data, ["URL", "Title", "Visits"]
        ...
        for f in os.listdir(pf_dir):
            if f.upper().startswith(exe) and f.endswith(".pf"):
                ...
                with open(path, "rb") as fd:
                    h = fd.read(8); info = f"MAM Compressed | {datetime.datetime.fromtimestamp(stat.st_mtime)}" if h[0:3] == b'MAM' else f"SCCA Standard"
                res.append((f, info))
